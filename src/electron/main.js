...
import { EHRAutopass } from '../core/ehr-autopass.js'
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    icon: join(__dirname, 'assets', 'icon.png'), // Add icon later
...
ipcMain.handle('ehr-initialize', async (event, options) => {
  try {
    ...
    ehrSystem = new EHRAutopass(options)
...
ipcMain.handle('ehr-join-provider', async (event, storagePath, providerId, invite) => {
  try {
    ...
    ehrSystem = await EHRAutopass.joinAsProvider(storagePath, providerId, invite)
    return { 
      success: true, 
